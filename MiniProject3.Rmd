...
varSummary <- function(.data = NULL, variable = NULL, rnd = 1, rname = NULL) {
  adsl_eff <- .data %>% 
    filter(EFFFL == "Y") %>%
    mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female"))  
  ...
  stat_cat <- adsl_eff %>%
    ...
    summarize(
            ...
            med = median({{variable}}) %>% round(digits = {{rnd}})   %>% format(nsmall = {{rnd}}),                              min = min({{variable}})    %>% round(digits = {{rnd}}-1) %>% format(nsmall = {{rnd}}-1), 
            max = max({{variable}})    %>% round(digits = {{rnd}}-1) %>% format(nsmall = {{rnd}}-1),
            n = n()) %>% 
...
```{r Weight}
adsl_eff_Weight <- varSummary(.data = adsl, variable = WEIGHTBL, rnd=3)
adsl_eff_Weight
```

```{r Height}
adsl_eff_Height <- varSummary(.data = adsl, variable = HEIGHTBL, rnd=3)
adsl_eff_Height
