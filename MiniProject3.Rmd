...
```{r function for final table}
varSummary <- function(.data = NULL, variable = NULL, rnd = NULL) {
  adsl_eff <- .data %>% 
    filter(EFFFL == "Y") %>%
    mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female"))  
  ...
  stat_cat <- adsl_eff %>%
    group_by(TRT01AN,TRT01A,SEX) %>%
    summarize(
            mean = mean({{variable}})  %>% round(digits = {{rnd}})   %>% format(nsmall={{rnd}})  ,
            sd = sd({{variable}})      %>% round(digits = {{rnd}}+1) %>% format(nsmall = {{rnd}}+1), 
            med = median({{variable}}) %>% round(digits = {{rnd}})   %>% format(nsmall = {{rnd}}),                              
            min = min({{variable}})    %>% round(digits = {{rnd}}) %>% format(nsmall = {{rnd}}), 
            max = max({{variable}})    %>% round(digits = {{rnd}}) %>% format(nsmall = {{rnd}}),
            n = n()) %>% 
    mutate(range_minmax= paste0("(",min, ", ", max, ")")) %>%
    ungroup() %>%
    select("TRT01A","SEX", "n", "mean", "med", "sd", "range_minmax") %>% 
    mutate(across(where(is.numeric), .fns = as.character)) %>%
    pivot_longer(cols = -c("TRT01A","SEX"), names_to ="category", values_to = "values" )%>%
    pivot_wider(names_from = c(TRT01A, SEX), values_from = values) %>%
    mutate(category = case_when(category == "n" ~ "N",
                                category == "med" ~ "Median", 
                                category == "mean" ~ "Mean", 
...
```{r Weight}
adsl_eff_Weight <- varSummary(.data = adsl, variable = WEIGHTBL, rnd=0)
adsl_eff_Weight
```

```{r Height}
adsl_eff_Height <- varSummary(.data = adsl, variable = HEIGHTBL, rnd=0)
adsl_eff_Height

```

```{r}
final <- rbind(adsl_eff_Weight, adsl_eff_Height)
final
```