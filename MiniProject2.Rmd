...
```{r small n and Big N merge}
adsl_mrg_cnt_2 <-
  small_n_cnt_2 %>%
  left_join(Big_N_cnt_2) %>%
  mutate(perc = round(n/N*100, digits=1)) %>%
  mutate(perc_char = format(perc, nsmall=1)) %>%
  mutate(npct = paste(n, "(", perc_char, ")" ) ) %>%
  mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female")) %>%
...
```{r final table challenge 1}
table_challenge_1 <- adsl_mrg_cnt_2 %>%
  pivot_wider(names_from = TRT01A, values_from = npct)
...
adae_saf <- adae %>% 
            filter(SAFFL == "Y" ) 
            #mutate(AEBODSYS = ifelse(AEBODSYS == "", "Missing", AEBODSYS)) 
adae_saf
```

```{r adae Big_N_cnt}
Big_N_cnt_3 <- adae_saf %>% 
             group_by(TRTAN, TRTA) %>% 
             count(name = "N")
Big_N_cnt_3
```
...
```{r adae smal_n_cnt}
small_n_cnt_3 <- adae_saf %>% 
             group_by(TRTAN, TRTA, AESOCCD, AEBODSYS) %>% 
             count(name = "n")
small_n_cnt_3
```

```{r adae final table}
adae_table_challenge2  <- small_n_cnt_3 %>%
             left_join(Big_N_cnt_3) %>% 
             mutate(perc = round(n/N*100, digits=1)) %>%
             mutate(perc_char = format(perc, nsmall=1)) %>%
             mutate(npct = paste(n, "(", perc_char, ")" ) ) %>%
             ungroup() %>%
             arrange(AESOCCD, AEBODSYS) %>% 
             select(TRTA, AEBODSYS, npct) %>% 
             pivot_wider(names_from = TRTA, values_from = npct)  %>% 
             mutate(AEBODSYS = ifelse(AEBODSYS == "", "Missing", AEBODSYS)) %>% 
             mutate(Placebo = ifelse(is.na(Placebo), "0", Placebo)) %>% 
             mutate(`Xanomeline Low Dose` = ifelse(is.na(`Xanomeline Low Dose`), "0", `Xanomeline Low Dose`))  %>% 
             mutate(`Xanomeline High Dose` = ifelse(is.na(`Xanomeline High Dose`), "0", `Xanomeline High Dose`))

adae_table_challenge2
```
