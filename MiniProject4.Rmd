...
sum(is.na(age_cat2$category))  # This will tell if any values are missing

# Arrange rows based on the ordered factor
sorted_age_cat2 <- age_cat2 %>%
  arrange(category)
```

```{r}
dm_allcomb2 <- bind_rows(sorted_age_cat2, agestat_cat)
...
```{r}
dm_allcomb3 <- rbind( dm_allcomb2[4,], dm_allcomb2[1:3, ], dm_allcomb2[5:8, ] )
...
#### Challenge 3. Add Ethnicity and Race.
#Ethnicity: "HISPANIC OR LATINO" and "NOT HISPANIC OR LATINO"
#Race: "ASIAN" and "WHITE"

```{r function for creating ETHNICITY and RACE category and n(%)}
...
```{r combination of first table with ETHNICITY and RACE}
...
dm_allcomb4 <- rbind("", dm_allcomb3)
dm_allcomb3[1,1] <- "Age Category, n (%)"

ethnicity <- rbind("", ethnicity)
ethnicity[1,1] <- "Ethnicity, n (%)"

race <- rbind("", race)
race[1,1] <- "Race, n (%)"

dm_allcomb_final <- rbind(dm_allcomb4, ethnicity, race)
dm_allcomb_final
```
